RayAABBIntersection ray_aabb_intersection(Ray ray, AABB aabb) {
  ...
  // Clamp the min so it's not negative, this way we don't intersect behind the ray.
  float tenter = max(max(traverse.x, traverse.y), 0.0);
