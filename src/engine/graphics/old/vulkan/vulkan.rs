...
impl VulkanInstance {
    pub fn new(config: VulkanConfig) -> Self {
        ...
        let instance = {
            let app_name = CString::new(constants::VULKAN_APP_NAME).unwrap();
            let engine_name = CString::new(constants::VULKAN_ENGINE_NAME).unwrap();
            ...
            let SwapchainSupport::Supported(has_display_handle, _) = config.swapchain_support
            else {
                panic!("[pyrite_vulkan]: Swapchain support is not enabled.")
            };
    ...
    pub fn default_queue(&self) -> &VulkanQueue {
        self.queue(constants::VULKAN_DEFAULT_QUEUE)
