...
use super::{fxaa::FxaaPass, voxel::pass::VoxelRenderPass};
...
pub fn refresh_render_resources(
    ...
    if !has_initialized {
        ...
        render_resource_manager.create_descriptor_sets(
            ...
            &fxaa_pass.descriptor_set_layout(),
        );
    }
}
...
pub fn get_fxaa_output_image<'a>(
    render_resource_manager: &'a RenderResourceManager,
) -> &'a dyn Image {
    render_resource_manager
        .get_image(gfx_constants::FXAA_IMAGE_NAME)
        .expect("fxaa image not created.")
}
